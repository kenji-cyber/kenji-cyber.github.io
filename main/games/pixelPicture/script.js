...
document.getElementById("pen").addEventListener('click',() =>{
     pincelPenClick("#87cefa","#ffffff",'mousedown');
   
})
document.getElementById("pincel").addEventListener('click',() =>{
   pincelPenClick("#ffffff","#87cefa",'mousemove');

})
let clickCont = 0;
let clickMax = 2;
pictureTable.addEventListener('click',() =>{
    clickCont++;
    setTimeout(() =>{
      clickCont = 0;
    },300);
    if(clickCont == clickMax){
        changePincelPen();
    }
  
})
function changePincelPen(){
    clickCont = 0;
      if(typePincel == 'mousedown' || typePincel == 'click'){
            pincelPenClick("#ffffff","#87cefa",'mousemove');
            clickMax = 1;
            }else if(typePincel == 'mousemove'){
                pincelPenClick("#87cefa","#ffffff",'mousedown');
                clickMax = 2;
            }
...
function pincelPenClick(color1,color2,mouseType){
    typePincel = mouseType;
    document.getElementById("pen").style.backgroundColor = color1; 
    document.getElementById("pincel").style.backgroundColor = color2;
    saveColor();
    remakeTable();
    pixelClick();
